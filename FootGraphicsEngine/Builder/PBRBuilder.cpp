...
namespace GraphicsEngineSpace
{
	...
	std::shared_ptr<IDXObject> PBRBuilder::BuildDXObject(std::shared_ptr<IDXObject> pDXObj, std::string objectName)
	{
		...
		{
			OutputDebugString(L"Not PBR OBJ");
	...
	{
		// Fake Shadow model 불러오기
		/*std::ifstream ifs("BinaryFile/Model/Shadow.model", std::ios_base::binary);
		boost::iostreams::filtering_streambuf<boost::iostreams::input> buffer;
		buffer.push(boost::iostreams::zlib_decompressor());
		buffer.push(ifs);
		boost::archive::binary_iarchive ia(buffer);

		FBXBinaryData::ModelData shadowModelData;
		...
		for (auto& mesh : modelData.meshInfoList)
		{
			...
			// 로더에서 뼈가 있으면 애니가 있다고 체크를 하고 넘겨야할듯
			// 그 다음 여기서 bone이 있지만 애니가없으면 static render로 넘기면될듯 isSkinnedMesh는 디폴트로 false 니까
			if(!modelData.animationClipList.empty())
		...
		//objRes->isShadow = false;

		////objRes->isLight = false;

		//objRes->ObjName = shadowModelData.meshInfoList[0];

		//DXObject->SetMeshResourcesFront(objRes);
		...
		for (auto& animClipName : modelData.animationClipList)
		{
			...
			// totaltime 계산..
			animClipRes->totalTime = (animClipRes->totalKeyFrame - animClipRes->startKeyFrame) * animClipRes->tickPerFrame;
