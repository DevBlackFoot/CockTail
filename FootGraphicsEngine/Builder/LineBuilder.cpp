...
namespace GraphicsEngineSpace
{
	...
	std::shared_ptr<MeshResources> LineBuilder::BuildGeometry(std::shared_ptr<IDXObject> DXObject, std::string objectPath)
	{
		...
		{
			_tmpObjRes->mesh = resourceManager->LoadMesh("WireCubeMesh", false);
		...
		{
			_tmpObjRes->mesh = resourceManager->LoadMesh("WireSphereMesh", false);
			_tmpObjRes->setMesh = true;
			std::dynamic_pointer_cast<LineObj>(DXObject)->SetHasColor(false);
			DXObject->SetMeshResources(_tmpObjRes);
		}
		...
		{
			_tmpObjRes->mesh = resourceManager->LoadMesh("WireCapsuleMesh", false);
			_tmpObjRes->setMesh = true;
			std::dynamic_pointer_cast<LineObj>(DXObject)->SetHasColor(false);
			DXObject->SetMeshResources(_tmpObjRes);
		}
	...
	{
		_objRes->mesh = resourceManager->LoadMesh("GridMesh", false);
	...
	{
		_objRes->mesh = resourceManager->LoadMesh("AxisMesh", false);
