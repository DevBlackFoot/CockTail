...
namespace GraphicsEngineSpace
{
	...
	private:
		...
		// 큐브맵 형식으로 만들어줄 쉐도우 큐브맵. (갱신할 DSV)
		std::shared_ptr<DepthStencilResource> pointShadowDSV;
		// 최초 한 번만 그려줄 DSV
		std::shared_ptr<DepthStencilResource> pointBakedShadowDSV;
		// SpotLight에 사용할 DSV (갱신할 DSV)
		std::shared_ptr<DepthStencilResource> spotShadowDSV;
		// 최초 한 번만 그려줄 DSV
		std::shared_ptr<DepthStencilResource> spotBakedShadowDSV;
	...
	public:
		...
		// 최초 한 번의 Bake
		void Bake(std::shared_ptr<IDXObject> object);
		// 사용한 리소스 초기화
		void EndRender();

		// Point Light용 함수를 따로 빼보자
		void PointRenderStart(int idx);
		void PointBakeStart(int idx);
		void PointRender(std::shared_ptr<IDXObject> object);
		void PointBake(std::shared_ptr<IDXObject> object);

		// Spot Light 용 렌더 함수
		void SpotRenderStart(int idx);
		// Bake map의 경우 그려주는 DSV가 다릅니다.
		void SpotBakeStart(int idx);
		...
		const std::shared_ptr<DepthStencilResource>& GetShadowDSV() { return dirShadowDSV; }
		const std::shared_ptr<DepthStencilResource>& GetPointShadowDSV() { return pointShadowDSV; }
		const std::shared_ptr<DepthStencilResource>& GetSpotShadowDSV() { return spotShadowDSV; }
		const std::shared_ptr<DepthStencilResource>& GetBakedPointShadowDSV() { return pointBakedShadowDSV; }
		const std::shared_ptr<DepthStencilResource>& GetBakedSpotShadowDSV() { return spotBakedShadowDSV; }
