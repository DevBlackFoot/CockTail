...
namespace GraphicsEngineSpace
{
	...
	private:
		// 외부에서 받아줄 인스턴스
		static std::shared_ptr<BuilderManager> instance;
	...
	public:
		...
		GRAPHICSENGINE_DECLSPEC static std::shared_ptr<BuilderManager> GetInstance()
		{
			if (instance == nullptr)
			{
				instance = std::make_shared<BuilderManager>();
			}
	...
	template <typename T>
	void BuilderManager::AddBuilder(std::string _builderName)
	{
		// 예외처리
		assert(D3DDevice);
