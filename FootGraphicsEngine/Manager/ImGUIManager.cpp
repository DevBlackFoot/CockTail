...
namespace GraphicsEngineSpace
{
	...
	bool ImGUIManager::Button(std::string label)
	{
		return ImGui::Button(label.c_str());
	}

	bool ImGUIManager::GetBool(std::string checkLabel)
	{
		auto parentID = ImGui::GetID(checkLabel.c_str());
		
		return ImGui::GetStateStorage()->GetBool(parentID);
	}
	...
	bool ImGUIManager::InputText(std::string label, char* text, unsigned bufferSize)
	{
		return ImGui::InputText(label.c_str(), text, bufferSize);
	}

	bool ImGUIManager::InputText(std::string label, std::string hint, char* text, unsigned bufferSize)
	{
		return ImGui::InputTextWithHint(label.c_str(), hint.c_str(), text, bufferSize);
	}
