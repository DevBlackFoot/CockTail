...
namespace GraphicsEngineSpace
{
	...
	void DepthStencilResource::CreateDepthStencil(ComPtr<ID3D11Device> _device, ComPtr<ID3D11DeviceContext> _immediateDC, int _clientWidth, int _clientHeight, bool
	                                              depthEnable)
	{
		...
		descDSS.DepthEnable = depthEnable;
		...
		HR(_device->CreateShaderResourceView(depthStencilBuffer.Get(), &srvDesc, depthShaderResource.GetAddressOf()));

		// Main DepthStencil Buffer만 Stencil SRV를 만들어줍니다.
		srvDesc.Format = DXGI_FORMAT_X24_TYPELESS_G8_UINT;
		HR(_device->CreateShaderResourceView(depthStencilBuffer.Get(), &srvDesc, stencilShaderResource.GetAddressOf()));
	...
	void DepthStencilResource::CreateDepthStencil(ComPtr<ID3D11Device> _device,
		ComPtr<ID3D11DeviceContext> _immediateDC, int _clientWidth, int _clientHeight, bool depthEnable,
		D3D11_COMPARISON_FUNC comparisionFunc)
	{
		// 여기 있는 매개 변수가 제대로 생성되었는지 확인.
		assert(_device);
		assert(_immediateDC);
		...
		descDSS.DepthEnable = depthEnable;
		descDSS.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
		descDSS.DepthFunc = comparisionFunc;
	...
	void DepthStencilResource::DeleteImmediateDepthStencil()
	{
		...
		if(depthShaderResource)
			depthShaderResource = nullptr;
