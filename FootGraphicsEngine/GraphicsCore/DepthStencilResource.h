...
namespace GraphicsEngineSpace
{
	...
	private:
		...
		// shader에 넣어줄 리소스
		ComPtr<ID3D11ShaderResourceView> depthShaderResource;
		ComPtr<ID3D11ShaderResourceView> stencilShaderResource;
	...
	public:
		...
		ComPtr<ID3D11ShaderResourceView> GetDepthShaderResource() { return depthShaderResource; }
		ComPtr<ID3D11ShaderResourceView> GetStencilShaderResource() { return stencilShaderResource; }
		...
		void CreateDepthStencil(
			...
			int _clientWidth, int _clientHeight, bool depthEnable);

		void CreateDepthStencil(
			ComPtr<ID3D11Device> _device, ComPtr<ID3D11DeviceContext> _immediateDC,
			int _clientWidth, int _clientHeight, bool depthEnable, D3D11_COMPARISON_FUNC comparisionFunc);
