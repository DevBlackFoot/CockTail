...
namespace GraphicsEngineSpace
{
	...
	VertexShader::VertexShader(ComPtr<ID3D11Device> _device, ComPtr<ID3D11DeviceContext> _deviceContext,
		const std::vector<uint8_t>& shaderByteCode, D3D11_INPUT_ELEMENT_DESC* desc, UINT elementsSize)
			: ShaderBase(ShaderType::VERTEX)
			, deviceContext(_deviceContext)
			, shader(nullptr)
			, layout(nullptr)
	{
		...
		if (CreateLayout(_device, shaderByteCode, desc, elementsSize) != true)
		{
			assert(0);
		}
	}
	...
	bool VertexShader::CreateLayout(ComPtr<ID3D11Device> _device, const std::vector<uint8_t>& shaderByteCode,
		D3D11_INPUT_ELEMENT_DESC* desc, UINT elementsSize)
	{
		HRESULT hr = _device->CreateInputLayout
		(
			desc,
	...
	bool VertexShader::CreateShader(ComPtr<ID3D11Device> _device, const std::vector<uint8_t>& shaderByteCode)
	{
		HRESULT hr = _device->CreateVertexShader
		(
			shaderByteCode.data(),
