...
// Unity Spec 성분을 가지고 오기 위한 노력..
	// 부도체의 specular 성분은 흰색에 가까운 회색이기 때문에 이 부분을 넣었습니다.
...
float CalcPointShadowFactor(SamplerComparisonState samShadow, int index, TextureCubeArray cubeShadowMap, TextureCubeArray bakedCubeShadowMap, float4 shadowPosH, float4 textureInfo, float range);
...
float3 BRDF(in float roughness2, in float metallic, in float3 diffuseColor, in float3 specularColor, in float NdotH, in float HdotV)
{
    ...
    return (diffuse_factor + (specularColor * specular_factor));
    ...
    in float3 albedo, in float ambientOcclusion, in float roughness, in float metallic, in float shadow)
{
    ...
    const float roughness2 = max(roughness * roughness, HALF_MIN_SQRT);
    const float roughness4 = max(roughness2 * roughness2, HALF_MIN);
    //const float roughness2 = max(roughness * roughness, 0.01f);

    // Blend base colors
    // 해당 부분을 point light와 비슷하게 잡아봅시다.
    const float3 c_diff = albedo * ambientOcclusion;
    ...
    float3 brdf_factor = BRDF(roughness4, metallic, c_diff, c_spec, NdotH, HdotV);

    // Directional light
    light.color = SRGBToLinear(light.color);
    acc_color += light.color.rgb * (light.power * NdotL) * shadow * brdf_factor;
    ...
    in float3 albedo, in float ambientOcclusion, in float roughness, in float metallic, in float shadow)
{
    ...
    for (uint i = 0; i < lightCount; i++)
    {
        ...
        float3 radiance = (Attn * NdotL) * light.power;
        ...
        // BRDF
        //float3 brdf_factor = BRDF(roughness2, metallic, c_diff, c_spec, NdotH, NdotV, NdotL, HdotV);
        float3 brdf_factor = BRDF(roughness2, metallic, c_diff, c_spec, NdotH, HdotV);
    ...
    in float3 albedo, in float ambientOcclusion, in float roughness, in float metallic, in float shadow)
{
    ...
    // products
    const float NdotL = max(saturate(dot(N, L)), EPSILON);
    const float NdotV = max(saturate(dot(N, V)), EPSILON);
    const float NdotH = max(saturate(dot(N, H)), EPSILON);
    const float HdotV = max(saturate(dot(H, V)), EPSILON);
    ...
    // Blend base colors
    // brdf 내부에서 metallic 처리를 해준다고 가정..
    const float3 c_diff = albedo * ambientOcclusion;
    //const float3 c_diff = lerp(albedo, float3(0, 0, 0), metallic) * ambientOcclusion;
    const float3 c_spec = lerp(kDielectricSpec.rgb, albedo, metallic) * ambientOcclusion;
...
float CalcShadowFactor(SamplerComparisonState samShadow, Texture2DArray shadowMap, Texture2DArray bakedShadowMap, float4 shadowPosH, float4 textureInfo, int idx)
{
	// NDC 기준 깊이값
    float depth = shadowPosH.z;
    ...
    {
        shadowFactor += shadowMap.SampleCmpLevelZero(samShadow, float3(shadowPosH.xy + offsets[i], idx), depth).r;
        shadowFactor2 += bakedShadowMap.SampleCmpLevelZero(samShadow, float3(shadowPosH.xy + offsets[i], idx), depth).r;
    }
...
// Point Light Shadow 계산
float CalcPointShadowFactorSam(SamplerState samShadow, int index, TextureCubeArray cubeShadowMap, float4 shadowPosH, float4 textureInfo, float range)
{
    // 깊이값
    float depth = CalcDepthInShadow(shadowPosH.xyz, range);
    ...
    shadowFactor = cubeShadowMap.Sample(samShadow, shadowPosH).r;
...
float CalcPointShadowFactor(SamplerComparisonState samShadow, int index, TextureCubeArray cubeShadowMap, TextureCubeArray bakedCubeShadowMap, float4 shadowPosH, float4 textureInfo, float range)
{
    ...
    shadowFactor = cubeShadowMap.SampleCmpLevelZero(samShadow, shadowPosH, depth).r;
    shadowFactor2 = bakedCubeShadowMap.SampleCmpLevelZero(samShadow, shadowPosH, depth).r;
