...
namespace GameEngineSpace
{
	...
	void Light::Awake()
	{
		...
		{
			currentScene->SetMainLight(this->shared_from_this());
			break;
		}
		default:
			break;
	...
	void Light::Update(float tick)
	{
		...
		{
			// view 생성
				// 엄청 큰 Spot light라고 생각하고..
				// 디렉션의 반대 방향으로..
			auto lightPos = Vector3{0.5f, 0.5f, 0.5f} - direction * 100.f;

			if (lightPos != Vector3::Zero)
			{
				Matrix view = XMMatrixLookAtLH(lightPos, Vector3{ 0.5f, 0.5f, 0.5f }, Vector3{0.f, 1.0f, 0.f});

				Vector3 newCenter = XMVector3TransformCoord(Vector3{ 0.5f, -1.f, 0.5f }, view);
				...
				Matrix view = XMMatrixLookAtLH(lightPos, lightPos + focusDir, transform->GetUp());
				...
				if (f < 1.f)
					f = 1.0001f;
				// 라디안으로 바꿔준 빛의 영역(각도)
				float degToRad = 2.0f * FootMath::Pi * (90.f / 360.f);
				if (degToRad == 0.f)
					degToRad = 0.0001f;
			...
			GraphicsManager::GetInstance()->PassDirectionalLight(color, direction, power, isShadow, lightViewProj);
		}
		break;
