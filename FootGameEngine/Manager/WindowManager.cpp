...
#include "../../CockTailClient/resource1.h"
...
namespace GameEngineSpace
{
	...
	LRESULT WindowManager::MsgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_MOUSEMOVE:
		{
			...
			//::SetCursor(cursor);
			if (presentCursor == false)
			{
				presentCursor = true;
				UIManager::GetInstance()->GetCanvas("CursorCanvas")->GetSpriteUI("Cursor")->SetEnable(presentCursor);
			}
	...
	void WindowManager::InitWindow(tstring windowCaption, HINSTANCE hInst, int screenWidth, int screenHeight)
	{
		...
		::SetCursor(cursor);
		*/
		//clickCursor = LoadCursor(hInst, MAKEINTRESOURCE(IDC_CURSOR1));
	...
	INT WindowManager::Update()
	{
		...
		while (TRUE)
		{
			...
			else
			{
				...
				{
					LoadingManager::GetInstance()->ShowLoadingImage();
				}
				else
				{
